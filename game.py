...
def computer_turn(board, comp, t_limit):
    """Makes comuter move.
    ...
    if (comp == 'R'):
        ...
        oth = 'R'
    while((time.time() - time1) <= t_limit and depth <= 42):
        depth += 1
        ...
        if(val != None):
            for child in root.children:
                if(child.value == val):
                    move = child.move
                    #print "val returned = %d, next move will be %d at depth %d" %(val, move, depth)
                    break
        else:
            depth -= 1
            #print "on early exit move = %d" %move
    print "Search took %0.1f s" %(time.time() - time1)
...
test = True
while(test):
    temp = raw_input("Enter the search time limit in seconds: ")
    try:
        int(temp)
    except ValueError:
        test = True
